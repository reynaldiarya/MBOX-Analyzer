...
st.set_page_config(
    page_title="MBOX Analyzer", layout="wide", initial_sidebar_state="expanded"
)
...
uploaded_file = st.sidebar.file_uploader(
    "Choose .mbox file", type=["mbox", "mbx"], help="Upload your MBOX file for analysis"
)
...
if uploaded_file is not None:
    if uploaded_file.name != st.session_state.uploaded_file_name:
        ...
        with st.spinner("Analyzing MBOX file..."):
            result = analyze_mbox(
                st.session_state.temp_file_path, top_n=settings.DEFAULT_TOP_N
            )
...
if st.session_state.temp_file_path and os.path.exists(st.session_state.temp_file_path):
    ...
    date_range = st.sidebar.date_input(
        "Date Range", value=[], help="Filter emails by date range"
    )
    ...
    if st.session_state.analytics_result:
        ...
        domain_options = sorted(
            list({e.sender_domain for e in result.emails if e.sender_domain})
        )

    sender_filter = st.sidebar.multiselect(
        "Filter by Sender", options=sender_options, key="sender_filter"
    )
    domain_filter = st.sidebar.multiselect(
        "Filter by Domain", options=domain_options, key="domain_filter"
    )
